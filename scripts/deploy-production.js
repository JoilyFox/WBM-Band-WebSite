...
console.log(`${colors.green}✓${colors.reset} Build found: ${colors.cyan}${buildPath}${colors.reset}`);
console.log(`${colors.green}✓${colors.reset} FTP Host: ${colors.cyan}${config.host}:${config.port}${colors.reset}`);
console.log(`${colors.green}✓${colors.reset} Remote Path: ${colors.cyan}${config.remoteRoot}${colors.reset}`);
console.log(`${colors.green}✓${colors.reset} Delete Remote: ${colors.cyan}${config.deleteRemote ? 'Yes' : 'No'}${colors.reset}`);
...
ftpDeploy.on('uploading', (data) => {
  ...
  if (progress >= lastProgress + 5 || progress === 100) {
    const fileName = data.filename.length > 50 
      ? '...' + data.filename.slice(-47)
      : data.filename;
