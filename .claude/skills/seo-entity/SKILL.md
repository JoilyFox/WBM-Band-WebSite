...
5. Per-song first-hand story → i18n key `releases.<slug>.story` (slug hyphens map to underscores). Renders into the MusicRecording `description` (machine-readable channel) **only**. A visible on-page `<MusicStory>` section was trialled on `/listen` and **removed by band preference** (2026-06), so the prose is bot-facing again — if reintroduced, keep the visible text and the JSON-LD description in sync. Never fabricate it.
...
- **Biggest content win = unique value-add prose** Genius can't have: the band's own meaning / recording notes / translation, in both locales, rendered visibly. Serves ranking, Genius-differentiation AND AI-answer citations at once. _Currently NOT rendered on-page_ — the `releases.<slug>.story` prose only feeds the JSON-LD description (the visible `<MusicStory>` trial was removed by preference). This remains the highest-value untapped on-page lever if the band wants it back.
...
- `components/music/Lyrics.vue` (H2 keyword) · `components/music/MusicDetailContent.vue` (`displayTitle` — must resolve the **localized** H1, see Gotchas) · `locales/{uk,en}.json` (`music.detail.lyrics_*`, `app.home_title`, `releases.<slug>.{description,story}`).
