  ...
  <div class="team-grid-wrapper relative z-10 pb-32">
    ...
    <div class="team-grid" :class="variantClass" aria-hidden="true">
      <div v-for="n in blocks" :key="n" class="member-block group" :class="`block-${n}`">
        <div
          class="inner flex items-center justify-center text-white/15 text-[0.65rem] tracking-widest uppercase opacity-0 group-hover:opacity-70 transition-opacity duration-500"
        >
          Block {{ n }}
        </div>
      </div>
...
<style scoped lang="scss">
  ...
  @media (min-width: 1024px) {
    ...
    .team-grid.layout-columns {
      grid-auto-rows: 88px;
    }

    /* Variant: columns (staggered vertical emphasis) */
    .team-grid.layout-columns :deep(.block-1) {
      grid-column: 1 / span 4;
      grid-row: 1 / span 3;
    }
    .team-grid.layout-columns :deep(.block-2) {
      grid-column: 1 / span 4;
      grid-row: 4 / span 4;
    }
    .team-grid.layout-columns :deep(.block-3) {
      grid-column: 5 / span 4;
      grid-row: 1 / span 4;
    }
    .team-grid.layout-columns :deep(.block-4) {
      grid-column: 5 / span 4;
      grid-row: 5 / span 3;
    }
    .team-grid.layout-columns :deep(.block-5) {
      grid-column: 9 / span 4;
      grid-row: 1 / span 3;
    }
    .team-grid.layout-columns :deep(.block-6) {
      grid-column: 9 / span 4;
      grid-row: 4 / span 4;
    }
  }
  ...
  .member-block {
    ...
    overflow: hidden;
    background:
      radial-gradient(circle at 35% 28%, rgba(255, 255, 255, 0.08), transparent 62%),
  ...
  .member-block .inner {
    transition: letter-spacing 1.2s cubic-bezier(0.16, 0.8, 0.26, 1);
  }
