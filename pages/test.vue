<template>
  <div class="animate-fade-in pt-20 p-6">
    <!-- Hero Section -->
    <section class="text-center py-12">
      ...
      <p class="text-xl text-surface-300 mb-8 max-w-2xl mx-auto">
        A modern full-stack application built with Nuxt 3, Vue 3, TypeScript, Tailwind CSS, and
        PrimeVue
      </p>
      ...
      <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
        <Button
          ...
          @click="showSuccess"
        />
        <Button label="Secondary Button" class="btn-secondary" icon="pi pi-cog" @click="showInfo" />
        <Button label="Outline Button" class="btn-outline" icon="pi pi-heart" @click="showWarn" />
        <Button
    ...
    <section class="mt-16">
      <div class="card max-w-md mx-auto animate-scale-in">
        <h3 class="text-xl font-semibold text-surface-50 mb-6 text-center">Try the Components</h3>

        <form class="space-y-4" @submit.prevent="handleSubmit">
          <div>
            <label for="name" class="block text-sm font-medium text-surface-200 mb-2"> Name </label>
            <InputText
              id="name"
              v-model="form.name"
      ...
      <div class="card max-w-4xl mx-auto animate-scale-in">
        ...
        <div v-if="apiResponse" class="mt-6">
          ...
          <div class="bg-surface-900/70 rounded-lg p-4 border border-surface-800">
            <pre class="text-sm text-surface-300 overflow-auto max-h-64">{{
      ...
      <div class="card max-w-4xl mx-auto animate-scale-in">
        ...
        <div class="p-4 bg-amber-900/20 rounded-lg border border-amber-800/30">
          ...
          <ul class="text-sm text-amber-300 space-y-1">
            <li>
              • <strong>Automatic 404 handling</strong> - Navigate to any unknown URL to see the
              error page
            </li>
            <li>
              • <strong>Custom error messages</strong> - Pass custom title, message, button text via
              query params
            </li>
            <li>
              • <strong>Multiple error types</strong> - Different icons and styling for different
              error scenarios
            </li>
            <li>
              • <strong>Easy programmatic redirect</strong> - Use the useErrorPage composable in
              your components
            </li>
            <li>
              • <strong>Glassmorphism design</strong> - Modern, responsive design that matches your
              app's theme
            </li>
          </ul>
        </div>
...
<script setup lang="ts">
  ...
  // Methods
  const showSuccess = () => {
    snackbar.success('Success!', 'Primary button clicked successfully')
  }

  const showInfo = () => {
    snackbar.info('Information', 'Secondary button clicked for more info')
  }

  const showWarn = () => {
    snackbar.warning('Warning!', 'Outline button clicked - proceed with caution')
  }
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (result) {
        apiResponse.value = {
          ...result,
          _meta: { duration: `${duration}ms`, cached: duration < 100 }
        }
        snackbar.success(
          'Users Fetched!',
  ...
  const fetchSlowData = async () => {
    ...
    try {
      ...
      if (result) {
        apiResponse.value = {
          ...result,
          _meta: { duration: `${duration}ms`, cached: duration < 2000 }
        }
        snackbar.warning(
  ...
  const clearAllCache = async () => {
    try {
      ...
      await refreshCacheStats()
      snackbar.success('Cache Cleared!', 'All cached API responses have been removed', 3000)
    } catch (error) {
  ...
  const clearUsersCache = async () => {
    try {
      ...
      await refreshCacheStats()
      snackbar.info('Users Cache Cleared!', 'Cached users data has been removed', 3000)
    } catch (error) {
  ...
  const clearPostsCache = async () => {
    try {
      ...
      await refreshCacheStats()
      snackbar.info('Posts Cache Cleared!', 'Cached posts data has been removed', 3000)
    } catch (error) {
  ...
  const redirectToCustomError = () => {
    return redirectToError({
      ...
      message:
        'This is a custom error message to demonstrate how you can pass your own title, message, button text, and navigation target.',
