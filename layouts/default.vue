<template>
  <div class="min-h-screen bg-surface-950">
    ...
    >
      <CommonContainer size="xl" padding="md" :overflow-visible="true">
        <div class="flex items-start justify-between h-16 relative pt-1">
          ...
          <div class="hidden md2:flex items-center justify-between h-full w-full">
            ...
            <div
              class="flex items-center gap-8 flex-1 justify-end transition-all duration-300"
              :class="leftNavSpacing"
            >
              <template v-for="(link, index) in leftNavLinks" :key="`left-${index}`">
                <button :class="navLinkClass" @click="() => handleNavClick(link)">
                  {{ t(link.label) }}
                </button>
              </template>
            </div>
            ...
            <div
              class="flex items-center gap-8 flex-1 justify-start transition-all duration-300"
              :class="rightNavSpacing"
            >
      ...
      <div
        class="hidden md2:block absolute top-[14px] left-1/2 transform -translate-x-1/2 z-50 transition-top duration-300"
          ...
          <div class="flex flex-col items-center mt-10">
            ...
            <nav class="flex flex-col items-center space-y-8">
              <template v-for="(link, index) in allNavLinks" :key="`mobile-${index}`">
                <button :class="mobileNavLinkClass" @click="() => handleMobileNavClick(link)">
                  {{ t(link.label) }}
                </button>
              </template>
            </nav>
...
<script setup lang="ts">
  ...
  const allNavLinks = computed(() => [...leftNavLinks, ...rightNavLinks])
  ...
  // Mobile menu icon
  const mobileMenuIcon = computed(() => (isMobileMenuOpen.value ? 'pi pi-times' : 'pi pi-bars'))
  ...
  // Methods
  const showWelcome = () => {
    snackbar.success(t('app.welcome', { name: bandName.value }), t('app.ready'), 5000)
  }
</script>
...
<style scoped>
  ...
  *:focus {
  ...
  .mobile-menu-enter-active .relative {
    animation: slideInUp 0.25s cubic-bezier(0.4, 0, 0.2, 1) both;
  }

  .mobile-menu-leave-active .relative {
    animation: slideOutDown 0.2s cubic-bezier(0.4, 0, 0.2, 1) both;
  }
